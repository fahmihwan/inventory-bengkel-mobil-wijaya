...
$barang_keluar_id = $_GET['id'];


$queryEditBarangMasuk = mysqli_query($conn, "SELECT * FROM barang_keluar
INNER JOIN montir
ON montir.id = barang_keluar.montir_id
WHERE barang_keluar.id='$barang_keluar_id'");
...
if (isset($_POST['submit-barang'])) {
    ...
    $tes =  mysqli_query($conn, "UPDATE barang_keluar
    SET montir_id='$montir',
    catatan='$catatan',
    tanggal='$tanggal'
    WHERE id='$barang_keluar_id'");
    ...
    try {
        ...
        for ($i = 0; $i <= count($cart) - 1; $i++) {
            if ($cart[$i]['transaksi_masuk_id'] == null) {
                ...
                $qty = $cart[$i]['qty'];
                mysqli_query($conn, "INSERT INTO transaksi_barang_keluar (barang_keluar_id, barang_id, qty) VALUES ('$barang_keluar_id','$barang_id','$qty')");
                ...
                if ($currentQty <= 0) {
                    echo "
                    <script>
                    alert('gagal, stok tidak cukup habis')
                    </script>
                    ";
        ...
        if ($delete_cart) {
            for ($j = 0; $j <= count($delete_cart) - 1; $j++) {
                if ($delete_cart[$j]['transaksi_masuk_id'] != null) {
                    ...
                    mysqli_query($conn, "DELETE FROM transaksi_barang_keluar WHERE id='$transaksi_id'");
        ...
        mysqli_commit($conn);
        echo "<script>
         window.location.href ='index.php?barang-keluar=detail&id=$barang_keluar_id';
        </script>";
    } catch (\Throwable $e) {
                ...
                <div class="card-body px-4">
                    ...
                    <div class="mb-3 clearfix">
                        <div class="" style="width: 100%;">
                            <input type="hidden" id="get-id-from-url" value="<?= $barang_keluar_id ?>">
                            ...
                            <div class="col">
                                <div class="mb-3">
                                    ...
                                    <select name="montir" id="select-supplier" class="js-example-basic-single-supplier form-control rounded-pill border-none py-2" style=" box-shadow: rgba(17, 17, 26, 0.1) 0px 1px 0px;">
                                        ...
                                        <?php } ?>
                                        <option value="default"> -- pilih montir-- </option>
                                        <?php while ($data = mysqli_fetch_assoc($queryAllMontir)) : ?>
                                            <option value="<?= $data['id'] ?>"><?= $data['nama'] ?></option>
                                        <?php
                                        endwhile;
                                        ?>
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            url: "pages/json-data/get_detail_barang_keluar.php?id=" + $('#get-id-from-url').val(),
            type: "GET",
            dataType: 'json',
                ...
                if (response.data != null) {
                    ...
                    barang.forEach(e => {
                        cartBarang.push({
                            ...
                            transaksi_masuk_id: e.transaksi_keluar_id,
